...
x <- read.csv("01_Input/N_Halogen_Compilation_ALL.csv")
...
x_allu <- filter(x_unfilt, !is.na(x$no3_as_n) & x$aquifer_grouped == "Quaternary")
x_allu_ND <- filter(x_allu, x_allu$no3_as_n_Cens == TRUE)
...
#####################################
mesozoic <- filter(x, x$aquifer_grouped == "Mesozoic")

group_means <- mesozoic %>%
  group_by(gwic_id) %>%
  summarise(
    mean_br = mean(br_mg_l, na.rm = TRUE),
    mean_cl = mean(cl_mg_l, na.rm = TRUE)
  )

salty_well <- filter(x, x$gwic_id == "130298")

#############################################
pdf(
  file = "03_Incremental/Cl_BrCl.pdf",
  height = 10,
  width = 10
)

# jpeg(filename ="03_Incremental/Cl_BrCl.jpeg", 
#      height = 10,
#      width = 10,
#      res = 300,
#      units = "in"
...
#Taking a look at all the data points from the well that exhibits a 5,000 cl:br mass ratio
# points(
#   y = salty_well$cl_mg_l/salty_well$br_mg_l,
#   x = salty_well$cl_mg_l,
#   pch = 16,
#   col = "black",
#   cex =1
# )
...
rect(
  xleft = 54,       #min cl from Karr et al., 2001 Swine Lagoon in NC
  ybottom = 58,    #min cl:br mass ratio from Hudak et al., 2003 cow goat horse dungn in TX
  xright =  1980,    #max cl from Panno et al., 2005 (Data tables supporting Panno et al., 2006)
  ytop =  1654,     #max cl:br mass ratiofrom Panno et al., 2005 (Data tables supporting Panno et al., 2006)
  lty=2,
  lwd =2,
  border  = "green")
...
#   "topleft",
#   pch = 16,
#   col = c("black","brown","purple"),
#   pt.cex = 2,
#   cex = 1.4,
#   legend = c("Animal Waste","Septic","Mix?")
# )

#Tried to extract some samples that were specifically septic or animal-waste affected
#In the end, no distinctive endmembers really emerged
# septic_ids <- c("323846","325721","158589")
# animal_ids <- c("263104","319527","252461", "210785","321057")
# both_ids <- c("325487","325720")
# 
# septic <- filter(x, x$gwic_id %in% septic_ids)
# animal <- filter(x, x$gwic_id %in% animal_ids)
# both <- filter(x, x$gwic_id %in% both_ids)

# points(
#   x = septic$cl_mg_l,
#   y = septic$cl_mg_l/septic$br_mg_l,
#   pch = 21,
#   cex = 2,
#   bg = "brown",
#   col = "black"
...
# points(
#   x = animal$cl_mg_l,
#   y = animal$cl_mg_l/animal$br_mg_l,
#   pch = 21,
#   cex = 2,
#   bg = "black",
#   col = "black"
...
check <- filter(x, (x$cl_mg_l/x$br_mg_l) > 2000)
############################
windows()
...
###########################
y <- read.csv("01_Input/SW_Corner_Geochem.csv")
